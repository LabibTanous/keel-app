...
interface ManualEntry { label: string; value: string }
type ManualGroup = { header: string; rows: ManualEntry[] };
      ...
      <Card style={{ padding: '2px var(--pad)' }}>
        {rows.map((r, i) => (
          <div key={r} style={{ display: 'flex', alignItems: 'center', minHeight: 50, padding: '12px 0', borderTop: i ? '1px solid var(--hairline)' : 'none' }}>
            ...
            <input
              placeholder="0"
              style={{ width: 90, textAlign: 'right', border: 'none', background: 'none', fontFamily: 'var(--font-ui)', fontSize: 15, color: 'var(--ink)', outline: 'none' }}
            />
          </div>
        ))}
        <div style={{ padding: '12px 0', borderTop: '1px solid var(--hairline)' }}>
          <button style={{ background: 'none', border: 'none', color: 'var(--pine)', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-ui)', padding: 0 }}>
            + Add another
          </button>
        </div>
      </Card>
