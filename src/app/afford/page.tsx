...
import { computeAfford } from '@/lib/engine';
import { Card, Disclaimer, money, moneyK, amt, Cur } from '@/components/keel/ui';
...
function EffectOnPlan({ cost, spendingLeft, bufferBalance, safeFloor, verdict, monthlyBuffer = 0 }: EffectProps) {
  const T = TIER_STYLE[verdict];
  const runwayTarget = safeFloor * 4.4; // approx 4-month target
  const fromBuffer = Math.max(0, cost - spendingLeft);
  ...
  const spendingTotal = spendingLeft; // full allocation this month (spendingLeft is what's left)
  const bufferColor = verdict === 'fits' ? 'var(--ink)' : verdict === 'dips' ? 'var(--gold)' : 'var(--clay)';

  // spending bar — out of spendingTotal
  const spendSegs: Segment[] = [
    { w: (fromSpending / Math.max(spendingTotal, 1)) * 100, c: T.color },
  ];
  // buffer bar — out of the full runway target
...
export default function AffordPage() {
  ...
  const safeFloor = profile.essentials; // one month of essentials is the safe floor
